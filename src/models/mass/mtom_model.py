...
from src.models.transportation.transportation_modes import (
...
def full_mtom_model(MTOM, p, b, c, R_prop_cruise, R_prop_hover, rho_bat):
    """Compute MTOM consistency check with current design parameters."""
    # Geometry
    ...
    V_cruise = cruise_speed(
        MTOM, p.g, c_l_cruise, c_d_cruise, p.alpha_deg_cruise, c, b, p.rho
    )
    ...
    # Climb Power
    T_req_total_climb = total_thrust_required_climb(
        D_climb, MTOM, p.g, p.theta_deg_climb
    )
    ...
    m_motor = motor_mass(
        P_req_total_hover, P_req_total_climb, p.n_prop_vert, p.n_prop_hor
    )
